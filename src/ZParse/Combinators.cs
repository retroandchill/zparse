...
public static class Combinators
{
    ...
    extension<TKind>(TokenListParser<TKind, Token<TKind>> parser)
    {
        ...
        public TokenListParser<TKind, T> Apply<T>(Func<Token<TKind>, TextParser<T>> valueParser)
        {
            ...
            return input =>
            {
                ...
                var uResult = uParser.AtEnd()(rt.Value.Span(input.Source));
                if (uResult.HasValue)
            ...
            where T : allows ref struct
        {
            ...
            return input =>
            {
                ...
                var uResult = valueParserAtEnd(rt.Value.Span(input.Source));
                if (uResult.HasValue)
                    return TokenListParserResult.Value(uResult.Value, rt.Location, rt.Remainder);
        ...
        where T : allows ref struct
    {
        ...
        {
            return Parse.Sequence(left.IgnoreThen(parser), right).Select(t => t.Item1);
        }
        ...
        /// </summary>
        /// <returns>The resulting parser.</returns>
        public TokenListParser<TKind, T> OptionalOrDefault()
        {
            ArgumentNullException.ThrowIfNull(parser);

            return parser.OptionalOrDefault(() => default!);
        }
        ...
        /// <returns>The resulting parser.</returns>
        public TokenListParser<TKind, T> OptionalOrDefault(Func<T> defaultValue)
        {
            ArgumentNullException.ThrowIfNull(parser);

            return parser.Or(input => TokenListParserResult.Value(defaultValue(), input, input));
        }
        ...
        /// <returns>The resulting parser.</returns>
        public TokenListParser<TKind, TProjection> SelectMany<TResult, TProjection>(
            Func<T, TokenListParser<TKind, TResult>> selector,
            Func<T, TResult, TProjection> projector
        )
            ...
            where TProjection : allows ref struct
        {
            ...
            return input =>
            {
                var rt = parser(input);
                if (!rt.HasValue)
                    return TokenListParserResult.CastEmpty<TKind, T, TProjection>(rt);

                var ru = selector(rt.Value)(rt.Remainder);
                return ru.HasValue
                    ? TokenListParserResult.Value(
                        projector(rt.Value, ru.Value),
                        input,
                        ru.Remainder
                    )
                    : TokenListParserResult.CastEmpty<TKind, TResult, TProjection>(ru);
            };
        }
        ...
        /// <returns>The resulting parser.</returns>
        public TokenListParser<TKind, TValue> Value<TValue>(TValue value)
        {
            ArgumentNullException.ThrowIfNull(parser);

            return parser.IgnoreThen(Parse.Return<TKind, TValue>(value));
        }
        ...
        /// <returns>The resulting parser.</returns>
        public TokenListParser<TKind, T> Where(
            Func<ReadOnlySpan<char>, T, bool> predicate,
        ...
        )
        {
            ...
            {
                var rt = parser(input);
                if (!rt.HasValue || predicate(input.Source, rt.Value))
                    return rt;
    ...
    extension<TKind, T>(TokenListParser<TKind, T> parser)
    {
        ...
        /// <param name="defaultValue">The default value</param>
        /// <returns>The resulting parser.</returns>
        public TokenListParser<TKind, T> OptionalOrDefault(T defaultValue)
        {
            ArgumentNullException.ThrowIfNull(parser);
        ...
        where T : allows ref struct
    {
        ...
        /// <param name="second">The second parser.</param>
        /// <returns>The resulting parser.</returns>
        public TextParser<TResult> IgnoreThen<TResult>(TextParser<TResult> second)
            where TResult : allows ref struct
        {
            ArgumentNullException.ThrowIfNull(parser);
            ArgumentNullException.ThrowIfNull(second);
            ...
            return input =>
            {
                var rt = parser(input);
                if (!rt.HasValue)
                    return Result.CastEmpty<T, TResult>(rt);
        ...
        /// <summary>
        /// Construct a parser that matches zero or one instance of <paramref name="parser"/>, returning the default value
        /// when no match is possible.
        /// </summary>
        /// <returns>The resulting parser.</returns>
        public TextParser<T> OptionalOrDefault()
        {
            ArgumentNullException.ThrowIfNull(parser);
        ...
        /// <param name="defaultValue">The default value.</param>
        /// <returns>The resulting parser.</returns>
        public TextParser<T> OptionalOrDefault(Func<T> defaultValue)
        {
            ArgumentNullException.ThrowIfNull(parser);

            return parser.Or(input => Result.Value(defaultValue(), input, input));
        }
        ...
        /// <param name="selector">A mapping from the first result to the second.</param>
        /// <returns>The resulting parser.</returns>
        public TextParser<TResult> Select<TResult>(Func<T, TResult> selector)
            where TResult : allows ref struct
        {
            ArgumentNullException.ThrowIfNull(parser);
            ArgumentNullException.ThrowIfNull(selector);
        ...
        /// The LINQ query comprehension pattern.
        /// </summary>
        /// <typeparam name="TInner">The type of the resulting value.</typeparam>
        /// <typeparam name="TResult"></typeparam>
        /// <param name="selector">A mapping from the first result to the second parser.</param>
        /// <param name="projector">Function mapping the results of the first two parsers onto the final result.</param>
        /// <returns>The resulting parser.</returns>
        public TextParser<TResult> SelectMany<TInner, TResult>(
            Func<T, TextParser<TInner>> selector,
            Func<T, TInner, TResult> projector
        )
            ...
            where TResult : allows ref struct
        {
            ...
            return input =>
            {
                ...
                var ru = selector(rt.Value)(rt.Remainder);
                return ru.HasValue
                    ? Result.Value(projector(rt.Value, ru.Value), input, ru.Remainder)
                    : Result.CastEmpty<TInner, TResult>(ru);
            };
        }
        ...
        /// <param name="second">The second parser.</param>
        /// <returns>The resulting parser.</returns>
        public TextParser<TResult> Then<TResult>(Func<T, TextParser<TResult>> second)
            where TResult : allows ref struct
        {
            ArgumentNullException.ThrowIfNull(parser);
            ArgumentNullException.ThrowIfNull(second);
    ...
    extension<T>(TextParser<T> parser)
    {
        ...
        /// <param name="defaultValue">The default value.</param>
        /// <returns>The resulting parser.</returns>
        public TextParser<T> OptionalOrDefault(T defaultValue)
        {
            ArgumentNullException.ThrowIfNull(parser);
