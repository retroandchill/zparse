...
{
    private static TextParser<char> Matching(
    ...
    {
        return Matching(
            parsed => char.ToUpper(parsed) == char.ToUpperInvariant(ch),
            Presentation.FormatLiteral(ch)
        );
    }
    ...
    /// </summary>
    public static TextParser<char> LetterOrDigit { get; } =
        Matching(char.IsLetterOrDigit, ["letter", "digit"]);
