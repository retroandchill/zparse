...
public static class Token
{
    ...
    public static TokenListParser<TKind, Token<TKind>[]> Sequence<TKind>(params TKind[] kinds)
    {
        ...
        TokenListParser<TKind, Token<TKind>[]> result = input =>
            TokenListParserResult.Value(new Token<TKind>[kinds.Length], input, input);
    ...
    /// <typeparam name="TKind">The type of the token being matched.</typeparam>
    /// <returns>A parser that will match tokens with the specified kind and value.</returns>
    public static TokenListParser<TKind, Token<TKind>> EqualToValueIgnoreCase<TKind>(
        TKind kind,
        string value
    )
    {
    ...
    /// <returns>The matched token.</returns>
    // ReSharper disable once MemberCanBePrivate.Global
    public static TokenListParser<TKind, Token<TKind>> Matching<TKind>(
    ...
    private static TokenListParser<TKind, Token<TKind>> Matching<TKind>(
        Func<TKind, bool> predicate,
