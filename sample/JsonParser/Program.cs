...
internal static class JsonTokenizer
{
    ...
    private static TextParser<Unit> JsonStringToken { get; } =
        from open in Character.EqualTo('"')
        from content in Character
            .EqualTo('\\')
    ...
    private static TextParser<Unit> JsonNumberToken { get; } =
...
// the JSON spec.
internal static class JsonTextParsers
{
    ...
    public static TextParser<double> Number { get; } =
        ...
        from exp in Character
            ...
            .Then(expsign =>
                Numerics.Natural.Select(n => double.Parse(n.ToStringValue()) * expsign)
            )
            .OptionalOrDefault()
...
internal static class JsonParser
{
    ...
    private static TokenListParser<JsonToken, object> JsonString { get; } =
        Token.EqualTo(JsonToken.String).Apply(JsonTextParsers.String).Select(s => (object)s);

    private static TokenListParser<JsonToken, object> JsonNumber { get; } =
        Token.EqualTo(JsonToken.Number).Apply(JsonTextParsers.Number).Select(n => (object)n);
    ...
    private static TokenListParser<JsonToken, object> JsonObject { get; } =
        from begin in Token.EqualTo(JsonToken.LBracket)
        from properties in JsonString
            .Named("property name")
            .Then(name =>
                Token
                    .EqualTo(JsonToken.Colon)
                    .IgnoreThen(
                        Parse
                            .Ref(() => JsonValue!)
            ...
            .ManyDelimitedBy(Token.EqualTo(JsonToken.Comma), end: Token.EqualTo(JsonToken.RBracket))
    ...
    static TokenListParser<JsonToken, object> JsonArray { get; } =
        ...
        from values in Parse
            .Ref(() => JsonValue!)
            .ManyDelimitedBy(
                Token.EqualTo(JsonToken.Comma),
                end: Token.EqualTo(JsonToken.RSquareBracket)
            )
        select (object)values;

    private static TokenListParser<JsonToken, object> JsonTrue { get; } =
        Token.EqualToValue(JsonToken.Identifier, "true").Value((object)true);

    private static TokenListParser<JsonToken, object> JsonFalse { get; } =
        Token.EqualToValue(JsonToken.Identifier, "false").Value((object)false);

    private static TokenListParser<JsonToken, object?> JsonNull { get; } =
        Token.EqualToValue(JsonToken.Identifier, "null").Value((object?)null);

    private static TokenListParser<JsonToken, object?> JsonValue { get; } =
        JsonString
            .AsNullable()
    ...
    private static TokenListParser<JsonToken, object?> JsonDocument { get; } = JsonValue.AtEnd();
...
internal static class Program
{
    ...
    private static void Print(object? value, int indent = 0)
    {
        switch (value)
        {
            case null:
    ...
    private static void Indent(int amount, string text)
    {
